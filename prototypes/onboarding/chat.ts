...
export interface OmiAppOptions {
  ...
  contextApp?: string;
  /**
   * A custom first message from omi, streamed in. Used by onboarding to welcome
   * the user in the assistant's own voice instead of the default orienting line.
   */
  opener?: string;
  onClose(): void;
}
...
export function openOmiApp(host: HTMLElement, options: OmiAppOptions): OmiChat {
  ...
  if (options.opener !== undefined) {
    addOmi({ text: options.opener }, true);
  } else addOmi(
    options.asking
      ? {
